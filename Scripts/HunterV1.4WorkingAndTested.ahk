...
Scriptlist:="Swamp Lizard|Red Salamander|"
...
Gui, Add, text, x12 y99 w65 h40 vstatus, %status%
...
If (script == "Red Salamander") {
	comeco2:
Loop{
	status("Aguardando Trigger")
	Pixelsearch,swampx,swampy,1189,450,1570,952,0x00ff00,3,fast ;se há itens no inv
	If (errorlevel == 0){
		status("Dropando Inv")
		mousegetpos,mousex,mousey
		random,rand1,-20,-5
		...
		Pixelsearch,itemchaox,itemchaoy,50,41,1088,684,0xcd0089,5,fast ;se ha itens no chao
			...
			If (errorlevel == 1){
				Pixelsearch,armadilhasprontasx,armadilhasprontasy,50,41,1088,684,0xff0000,15,fast ;achando armadilhas catched
					If (errorlevel == 0){
					status("Checando Armadilha Amarela")
					mousegetpos,mousex,mousey
					random,var1,15,20
					...
					If (errorlevel == 1){
						Pixelsearch,armadilhasemvermelhox,armadilhasemvermelhoy,50,41,1088,684,0x43ff00,3,fast ;achando armadilhas em vermelho
							If (errorlevel == 0){
							status("Armadilha Vermelha")
							loop 8{
							Pixelsearch,itemchaox,itemchaoy,50,41,1088,684,0xcd0089,5,fast ;acha itens em roxo ate acabarem
